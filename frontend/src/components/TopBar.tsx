  ...
  return (
    <header className="h-16 bg-white border-b flex items-center justify-between px-4 md:px-8">
      <div className="flex items-center gap-3">
        <button onClick={onMenuToggle} className="lg:hidden p-2">
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          ...
          className="flex items-center gap-2 border px-3 py-2 rounded-lg hover:bg-slate-50 transition-colors"
        >
          {uploadState === "idle" && <Upload className="w-4 h-4" />}
          {uploadState === "uploading" && <Loader2 className="w-4 h-4 animate-spin" />}
          {uploadState === "success" && <CheckCircle2 className="w-4 h-4 text-green-600" />}

          <span>
            {uploadState === "idle" && "Upload Financials"}
            {uploadState === "uploading" && "Uploading..."}
            {uploadState === "success" && "Success"}
          </span>
        </button>

        <div className="hidden md:flex items-center gap-2 border px-3 py-2 rounded-lg">
